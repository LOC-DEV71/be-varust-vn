...
// ✅ CORS config
app.use(cors({
  origin: ["http://localhost:3000", "https://varust-vn.vercel.app"],
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (dup.length) {
      return res.status(409).json({ success: false, message: "Username, email hoặc phone đã tồn tại" });
    }
